...
当前版本：程序 **v1.11.2**、扩展 **v1.11.2**（程序扩展版本永远钉齐；这轮扩展加了后台标签页
防冻结 C6，必须重装一次才生效）。
...
4. **v1.11.2 已出包（2026-08-01），欠用户真机验收**：程序和扩展 zip 都在 `release/`（两个都是
   v1.11.2）。**换程序 + 重装一次扩展**（覆盖时别删 `dcwatch.db`）。验新东西：
   后台标签页防冻结（C6：挂后台久不再漏消息，图标悬停看「救回过 N 个」）、
