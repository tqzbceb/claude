...
当前版本 **v1.9.3**（程序）/ **v1.8.0**（扩展，两者版本号本来就不一样）。
...
4. **欠用户一次发版 + 真机验收**（2026-07-31 现状）：A1–A5 全修完但**都没发版**，
   用户机器上跑的还是旧版。下一件大事是出 v1.9.4（A2 动过 extension，
   要 bump 三处版本号、程序扩展两个 zip 都出），然后请用户真机验三条
   （见 `HANDOFF.md` 当前状态第 2 条）。另外他机器上的规则填得对不对，
