...
当前版本：程序 **v1.11.1**、扩展 **v1.11.1**（v1.11.1 起程序扩展版本永远钉齐；这轮扩展只改
版本号、功能没动，但要重装一次才显示新号）。
...
4. **v1.11.1 已出包（2026-08-01），欠用户真机验收**：程序和扩展 zip 都在 `release/`（两个都是
   v1.11.1）。**换程序 + 重装一次扩展**（覆盖时别删 `dcwatch.db`；扩展只是版本号钉齐、功能没动）。验新东西：
