...
当前版本：程序 **v1.11.3**、扩展 **v1.11.3**（程序扩展版本永远钉齐；这轮扩展有功能改动——
行内表情解析 D1、自动开帖改新开最小化窗口 D2，必须重装一次才生效）。
...
4. **v1.11.3 已出包（2026-08-01），欠用户真机验收**：程序和扩展 zip 都在 `release/`（两个都是
   v1.11.3）。**换程序 + 重装一次扩展**（覆盖时别删 `dcwatch.db`）。验新东西：
