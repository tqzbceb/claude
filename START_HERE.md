...
当前版本：程序 **v1.10.0**、扩展 **v1.9.4**（v1.10.0 没动扩展，用户不用重装扩展）。
...
4. **v1.10.0 已出包（2026-08-01），欠用户真机验收**：程序 zip 在 `release/`（扩展还是 v1.9.4 那份，
   这轮没动）。等用户装上后验新东西：拉模型（A6 修了他两台全 401 的问题）、规则的「AI 复核」开关（B1）、
   采样参数 + 后处理三档（B4/B2）、提示词抽屉可编辑（B3）。早前三条也别丢：A2 回复消息的盯人命中、
   A3 两个标签页只弹一条、A5 关三个开关立刻全停。
   另外他机器上的规则填得对不对，
