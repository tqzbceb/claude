...
## 最后更新时间
2026-08-01 · v1.11.3 出包轮（W+X 窗口）—— D1 行内表情 / D2 开帖新开最小化窗口 / D3 开服监听，代码侧清零
...
## 当前状态
**v1.11.3 已出包（2026-08-01，W+X 窗口）**：D1（行内表情不丢 + 剥 ~N 后缀）+ D2（自动开帖
改新开**最小化窗口**）+ D3（**开服监听**新页）三条落盘并一次出包。
程序 / EXT_MIN / manifest 三处钉齐 **v1.11.3**（硬规矩 2）。
回归：服务端 13 件 **704 条全绿**（新增 e2e_watch 26）+ 浏览器侧 107 条（run 49 / fresh 16 / tabs 42）全绿。
程序 zip（27 文件）+ 扩展 zip（10 文件）都在 `release/` 和 outputs/，旧 1.11.2 两个包已删。

下一件按顺序：
1. **等用户装 v1.11.3 真机验收 + 要新诊断包**（**程序扩展都要换**：扩展有功能改动，必须重装）。
   验新三条：①D1 行内表情留在正文里（让人发条「文字夹表情」的消息看收信箱）、`:cat_cry~1:`
   变 `:cat_cry:`；②D2 开「自动点开新帖」后新帖开在**新开的最小化窗口**（不抢焦点、闲置自关时
   窗口跟着关）；③D3 侧栏「⌖ 开服监听」加一个目标，状态翻脸时走通知管线提醒。
   旧两条别丢：C6 挂后台久不漏消息（图标悬停「救回过 N 个」）、A7 没规则=零收信。
   连带老欠账：A6 密钥重填后拉模型、B1 AI 复核。
2. 用户新反馈（验收后大概率有）。
**代码侧 BACKLOG 已清零。**
