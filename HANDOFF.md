...
2. **先跑一次回归确认环境是好的**：`pip install aiohttp`，然后
   `cd tests && ./runall.sh e2e.py e2e_wb.py`（应为 46 / 67 全绿）。已在干净 clone 上实测过，
   不绿就是环境问题，不是代码问题。
...
- **交接被第二个 run 独立复验过**（不是同一个写它的 run）：全新 `git clone` 到空目录，
  只装 aiohttp，`cd tests && ./runall.sh e2e.py e2e_wb.py` → 46 / 67 全绿。
  同时补掉了三个「新人只能靠猜」的缺口：README 第一屏就把接手的 AI 指向 `AGENTS.md`；
  `AGENTS.md` 新增「怎么出交付包给用户」（26 个文件的准确清单 + 命令，实跑出来和已交付的
  `dcwatch-v1.8.0.zip` 同样是 26 文件 / 345KB）；README 新增第十一节讲换号两条路和第一句话怎么说

## 未完成（按优先级排序）
- [ ] **先要一份诊断包**（见最上面第 3 条）。到 2026-07-31 为止都不知道用户机器上跑的是哪一版、
