...
## v1.12.2（AP/AQ/AR/AS 窗口，2026-08-02）：F2 类型闸（人 vs 频道）+ F3 折叠

**用户原话**：「模型好像分不清用户 id 和频道 id，我每次都要跟他解释，他还是搞错，我说监听某个
用户，他给我建成监听频道，我等半天没消息。他不确定的时候就该问我，不许直接生成」＋「这表得可以折叠」。

**根因**：snowflake 不管人还是频道都是 17–19 位数字，光看 ID 判不了类型；而 `create_rule` /
`update_rule` **收下就存、压根不校验**。规则语法合法、程序照跑，用户干等半天 —— 它在盯一个
不存在消息的「频道」。F1 的 `names` 表其实存了 kind，只是没人去对。

**修法（四道闸，`2535e0f` + `a9278a6` + `f6c4a8b` + `1d4a0fb`）**：
- 服务端 `SLOT_OK`/`KIND_SLOT` + `kinds_of_id`/`id_label`/`check_slot_kinds`/`rule_targets_cn`：
  栏位与类型不符**拒绝保存**并点名正确栏位；分类填进 `channel_ids` 也拦（永远不触发的经典坑）；
  名录没见过的不拦，但回「类型不确定，必须跟用户确认」。
- `find_target` 支持**数字反查**（这串 ID 是人还是频道）；四处提示词写死「拿不准先问用户」
  （RULE_FIELDS_DOC / WB_TOOLS_HOWTO 纪律 5b / WORKBENCH_SYS 功能清单 / WIZARD_SYS）。
- 界面：`/api/state` 和保存/删除/导入响应带 `idlabels`，规则卡片把 ID 翻成「频道 公告」，
  填错栏位波浪红线；`/api/rules` 保存前过闸，红框列出错处，留一条「名录认错了、仍然保存」
  的退路（`force_slots`；开关规则也带上，免得停用被闸拦成"按了没反应"）。
- F3：挑频道抽屉的树做成可折叠块（默认收起、`localStorage` 记状态、全部展开/收起，
  加 ID 按钮 `stopPropagation` 免得顺手折叠），云浏览器实测持久化。

**复核抓到的真 bug（`1d4a0fb`，差点让整道闸白干）**：模型照规矩用 `find_target` 从名录查到的
ID（侧栏抄来、那人还没说过话）会被另一道**旧的「防编 ID」闸**当成假 ID **悄悄丢掉**，规则被洗成
「全部消息」—— 用户又是等半天等来一堆不相干的。修法：`rule_ctx` 的 known 并进 `names_ids()`，
`run_wb_tool` 建/改规则时再兜一次底。**教训：加新闸之前先数清楚同一条路上已经有几道闸。**

**出包（AS 窗口）**：三处钉齐 v1.12.2；服务端 14 件 **907 条全绿**（比 870 多 37 条＝`e2e_name`
第 11 节 F2 常存回归：数字反查 / 人填频道栏拦 / 频道填人栏拦 / 分类拦 / 名录 ID 留得住 /
编的 ID 照旧丢 / 界面过闸与 force_slots / idlabels / 提示词纪律）；双 zip 在 `release/`
（程序 27 文件、扩展 11 条目，逐条跟 1.12.1 清单对齐，旧包已删）+ `outputs/`；全新解压冒烟
（启动首行 `dcwatch v1.12.2`、`/version`、`/api/presets`、`/api/names`、`/api/lookup`）全过。
**这一版扩展代码零改动**，只跟着钉版本号，用户不重装也能用。
