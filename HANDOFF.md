...
## 最后更新时间
2026-07-31 17:40（北京时间）

## 这一轮最后干的事：把「换号还能接上」做成了仓库里的东西

用户 2026-07-31 第二次说**这个窗口是最后一个**（账号限额，要换 project 拿新额度，
新号是新工作区，我的记忆一个字都过不去）。所以这一轮收尾没写功能，写的是交接本身：

- 新增 `START_HERE.md` —— 换号后第一个读的页。头四件事、用户的硬约束、凭据怎么要、
  怎么打交付包、这个云环境有哪些坑（`/tmp` 会被清、工作区同步会吃掉 `.bat`、`setsid` 那个坑），
  **以及「以后再换号该把哪段话发给新窗口」的原文**。
- 新增 `release/dcwatch-v1.9.1.zip` —— 打好的成品包进了仓库。用户**不需要任何 AI**
  也能从 GitHub 网页 Download raw file 拿到能跑的程序（旧版 zip 只留最新一个，见 `release/README.md`）。
- `AGENTS.md` / `README.md` / `CLAUDE.md` 的第一屏都改成指向 `START_HERE.md`；
  README 第十一节里那段「新会话第一句话」换成了现在这版（更短、指向 START_HERE）。

**接手的你现在该做的第一件事仍然是**：跟用户要一份诊断包（界面「运行日志」页 → 导出诊断），
看 `[0] 一眼结论` 和 `[4.5] 拿真实消息试算` 两段。要推代码时跟他现要一个 GitHub PAT。

## 当前状态
dcwatch **v1.9.1**。**扩展还是没改**，留在 v1.8.0，`EXT_MIN` 也留在 1.8.0 ——
用户**不需要重装扩展**，只换程序文件夹即可（硬规矩 2：扩展没动就别让他白重装一遍）。

本轮做的是上一轮「未完成」里**第一项能自己动手的事**：**给导入来的规则留痕迹**
（前两项「要诊断包」「等他装 v1.9.0 回执」都只能等用户，不是活）。硬规矩 5 的直接应用：
v1.9.0 加了导入功能却没在诊断包里留任何线索，他拿别人一份规则包导进去以后，
排查「这条规则我怎么不记得填过」时缺一条关键线索。

回归**服务端 416 条全绿**（e2e 46 + ai 27 + multi 26 + wiz 83 + v17 46 + diag 47 + wb 67 +
**imp 74，本轮从 59 加到 74**）。`extension/` 一行没动，浏览器侧那 43 + 16 条没重跑。
界面那个「导入」小标签在真浏览器里渲染过（CDP 喂两条规则进 `renderRules()`，
标签文字和 title 都对）。交付 `outputs/dcwatch-v1.9.1.zip`（26 文件 / 353KB，
解压起服务实测过：导入后诊断包 [4] 段真印出来路），同一份也提交进了 `release/`。
**没有出扩展 zip**（扩展没改）。
...
- **v1.9.1（本轮）导入来的规则会留痕** ——
  · 导入落库时给规则盖两个本机字段 `imported_at`（时间）和 `imported_from`（包的版本号）。
    **故意不放进 `DEFAULT_RULE`**，所以 `rule_for_export()` 不会把它导出去：跟 hits 一样是本机的账，
    别人拿到我的规则包不该看见我是从哪一版导进来的。`IMPORT_MARKS` 就是这两个名字
  · `dry_run` 预览不盖戳（只在真写库那一支盖）；新增和覆盖都盖；`same` 的不动库也不盖
  · `POST /api/rules`（界面保存）会**从库里把这两个字段捞回来续上** —— 表单里没有对应输入框，
    提交的 body 里根本不带它们，不续就等于用户点一次「编辑」就把痕迹擦了
  · `sanitize_import_rule()` 把这两个名字加进「不当未知字段抱怨」的白名单（手工改过的文件也不会刷警告）
  · 诊断包 `[4]` 段：段首多一行「其中 N/M 条是导入来的，最近一次导入 <时间>」，
    每条导入来的规则末尾多一行「来路 <时间> 从规则包导入（v1.9.0）」
  · 界面「监听规则」列表：导入来的规则名字后面一个「导入」小标签，title 是时间 + 来源版本
  · `tests/e2e_imp.py` 加第 13 节（15 条，59 → 74）；README 第三节 + 故障对照表各补一条

## 未完成（按优先级排序）
- [ ] **用户换号后第一次连上时，确认他手上有 v1.9.1**：让他打开
      `http://127.0.0.1:8777/version` 念给你，或直接从
      `https://github.com/tqzbceb/claude/tree/main/release` 下最新 zip 覆盖（扩展不用动）
- [ ] **先要一份诊断包**（见最上面第 3 条）。到 2026-07-31 为止仍不知道用户机器上跑的是哪一版、
      规则改对没有 —— 这是唯一真正卡住的事，其余都是可选改进。他不会命令行，别让他敲命令
- [ ] 等用户装上 **v1.9.1**（**扩展不用动，还是 v1.8.0**）并确认两件事：
      ①「我为什么收不到消息」直接在工作台问，它会自己查状态并给结论；②让它改一条规则，看动作行有没有 `✔`。
      不通就要一份新诊断包，先看 [0] 和 [4.5] 两段
- [ ] **下一件能直接动手的：**让工作台的模型也能用导入/导出（`WB_TOOLS` 现在只有列/建/改/启停/删/试算）。
