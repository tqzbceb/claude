...
## 最后更新时间
2026-07-31 17:16（北京时间）

## 上一轮是「换号收尾轮」，先看这三行
- **用户的 Browser Use 账号到额度上限了，他要换 project / 换号**。那一轮是旧账号最后一个窗口。
- 换号前核对过：工作区那份工作副本和远端 `6de16e7` **零差异**（只少了同步会吞掉的
  `启动.bat` / `停止.bat` / `build.bat` / `tests/runall.sh`）。**没有任何代码只存在于旧工作区**。
- 干净 clone 上现场跑过 `e2e.py` + `e2e_imp.py`：**46 / 74 全绿**。所以你读到的这份仓库是能跑的。
- 那一轮唯一的实质改动：`START_HERE.md` 新增 **第 5.5 节** ——
  接手后必须把项目指针写进**新工作区根目录的 `AGENTS.md`**。原因见下面「注意事项」倒数第二条。
  **如果你是新账号第一个会话，这件事现在就做，别往后拖。**

## 当前状态
dcwatch **v1.9.2**。**扩展还是没改**，留在 v1.8.0，`EXT_MIN` 也留在 1.8.0 ——
用户**不需要重装扩展**，只换程序文件夹即可（硬规矩 2）。

本轮做的是上一轮「未完成」里第一件能自己动手的事：**工作台的模型多了 `export_rules` 这只手**
（前两项「要诊断包」「等他装新版回执」只能等用户）。**刻意没给 `import_rules`** —— 见下面注意事项。

回归**服务端 443 条全绿**（e2e 46 + ai 27 + multi 26 + wiz 83 + v17 46 + diag 47 +
**wb 94，本轮从 67 加到 94** + imp 74）。`extension/` 一行没动，浏览器侧那 43 + 16 条没重跑。
交付 `outputs/dcwatch-v1.9.2.zip`（26 文件 / 356KB），同一份也换进了 `release/`（旧的 1.9.1 删了）。
**交付包解压后真起了一次服务**：让假模型调一次 `export_rules`，喂回给它的包里没有那条规则的
转发密址、打码说明在、`/version` 写 1.9.2。**没有出扩展 zip**（扩展没改）。
...
- [ ] **先要一份诊断包**。到 2026-07-31 为止仍不知道用户机器上跑的是哪一版、规则改对没有
      ...
      他不会命令行，别让他敲命令
- [ ] 等用户装上 **v1.9.2**（**扩展不用动，还是 v1.8.0**）并确认三件事：
      ①「我为什么收不到消息」直接在工作台问，它会自己查状态给结论；②让它改一条规则，
      看动作行有没有 `✔`；③跟它说「把我的规则整包给我一份」，看它是不是真念出来了。
      不通就要一份新诊断包，先看 [0] 和 [4.5] 两段
- [ ] **下一件能直接动手的：**「批量提取」的模板（`extract_templates`）现在只能手填，
      也搬不走。照规则包那一套做成能导出 / 导入的包（**同一道预览闸**，硬规矩 10），
      并复用 `RULES_SCHEMA` 的思路给它一个 `dcwatch.extract/1`
