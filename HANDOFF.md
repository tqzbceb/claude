...
## 当前状态
dcwatch **v1.9.3**。**扩展还是没改**，留在 v1.8.0，`EXT_MIN` 也留在 1.8.0 ——
...
- [ ] **下一件能直接动手的**：目前没有明显的下一个功能。可选方向（都不急）：
      工作台给模型加只读的 `export_extract_templates`（照 export_rules 那套打码 / 限长）；
      或把批量提取的结果表也做成能存成「一次提取记录」回看。先等用户用 v1.9.3 的反馈再定
