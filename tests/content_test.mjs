...
export async function runTabs(session) {
  ...
  // 在页面里装 chrome 桩 + 复刻 background.js 的 tabOrders 核心逻辑，跑多组场景
  const r = await session.Runtime.evaluate({
    ...
    expression: `(() => {
      ...
      // ---- chrome 桩 ----
      let nextId = 100;
      const liveTabs = new Map();          // tabId → {id,url}
      const storage = { opened: {}, st: {}, port: 8777, bridge: "test-br" };
      const fetchLog = [];                // {url, body}
      let hbResp = null;                  // 下一次 /api/ext/hb 的响应
      let tabsResp = { ok: true, open_now: 0 };
      let createShouldFail = null;        // tid → err string，模拟 create 失败
      ...
      const chrome = {
        ...
        tabs: {
          ...
          query: async (q) => {
            ...
            if (q && q.url) {
              // 简化：只认 "*://discord.com/*"
              arr = arr.filter(x => /discord\\.com/.test(x.url || ""));
            }
            return arr;
          },
          onRemoved: {
      ...
      // ---- 复刻 background.js 的辅助 + tabOrders（与源码同逻辑） ----
      ...
      // onRemoved 监听器（与 background.js 同逻辑）
      ...
      // 1) 心跳带 open → 真调 create，storage 记 opened
      ...
      // 3) create 失败 → 进 failed，不写 opened
      ...
      rep = await tabOrders();
      t("create 失败进 failed", rep.failed.some(f => f.tid === T2 && /gesture|Error|required/i.test(f.err)),
        rep);
      ...
      // 9) auto_open=false 时 st.aoOn 写成 false（扩展仍执行指令——关总闸在服务端）
      ...
      // 12) chrome.tabs 桩本身：query / get / remove 基本行为
      liveTabs.clear();
      ...
      return { pass: checks.filter(c => c.ok).length, fail: checks.filter(c => !c.ok).length,
               failed: checks.filter(c => !c.ok), total: checks.length, checks };
    })()` });

  if (r.exceptionDetails) {
    return { boot: "FAIL", error: r.exceptionDetails.text || JSON.stringify(r.exceptionDetails) };
  }
