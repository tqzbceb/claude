...
## 状态
**PLAN_A4 已出，等实现窗口施工**（快模型接到「继续」：读 `PLAN_A4.md` 照施工图干，只动 ui.html）。
根因已定性（Claude 实测）：出口类型选择器**功能没坏**（演示+真鼠标+curl 全链路验证过），
坏的是 UX——类型下拉没标签且排在按钮后面、行内没有换类型入口、模板地址被密码框打码。
修法全在 PLAN_A4.md，四处改动 + 目检清单 + 回归要求。
