...
## 状态
**正在出 PLAN_A4**（Claude 窗口，2026-07-31）：A4 = 通知与转发的「出口」锁死 Discord
（选了 Discord 后其他类型选不了；「+添加出口」只是复制 Discord 配置）。
步骤：① 查 ui.html 出口编辑器根因 → ② 写 PLAN_A4.md（精确到行号）→ ③ NOW 指向 PLAN 推送。
中途被掐断的话：下个 Claude 窗口从 ① 接着做，别让快模型上。
