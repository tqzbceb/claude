...
## P 窗口（第十六个，2026-08-01）认领：A7（收信箱无规则也收信）+ C5（输入框空盒光标两处）

用户真机验收 v1.11.0 期间报的两条新问题，已先写进 BACKLOG（「新反馈」节）再动工：
- 工作区又是新的：clone ✓、五文档读完 ✓、根 AGENTS.md 指针 ✓、PAT 长存 secrets/ ✓
  （用户这次贴的是 token1 …0Dx28of，可推）、环境自检 e2e 50/50 + imp 74/74 全绿 ✓、
  远端核实 HEAD=2f0038e 无人撞车 ✓
- 顺序：A7（server.py 落库闸 + 诊断印跳过数 + 测试）→ C5（ui.html 输入框，先读 C4 现状）。
  两条都是小程序侧改动；不动 extension/、不 bump 版本号（硬规矩 2）。
- A7 设计（照用户原话「我写什么规则他就收什么信」）：落库前过「规则覆盖闸」——
...
`match()` 重构为 covers()+听内容（试算原因文案不变）；`_handle_event` 落库前过闸：
**零规则 = 零收信**；扩展自检消息（channel_id 固定 "0"）豁免；抓历史走 store_only 不过闸；
C2 自动开帖排队在闸前照常。拦下数记 `_gate_n`：诊断 [0] 结论条点名、诊断包 [3] 段固定印出 →
② **C5** —— 根因是 C4 留的 composer「点空白处=点输入框」点击监听，删掉；
空盒只剩 textarea 单行一处可聚焦（顶左上角）→ ③ 测试适配：e2e.py +3 条 A7 断言
（零规则不收/兜底罩住才收）；e2e_multi/diag/wiz/tabs 各补「罩得住但永不提醒」兜底规则 →
④ 全套回归 **678 条全绿**（e2e 53 · ai 27 · multi 26 · wb 118 · imp 74 · ext 53 · v17 46 ·
diag 47 · chat 42 · wiz 83 · tabs 53 · chk 56）→ ⑤ README 三处（ID 拿法的先有鸡说明 /
[3] 段说明 / 故障表加「收信闸」行）+ NOW/BACKLOG 收尾推送。
**没有进行中的任务，代码侧又清零。** 下一件全在用户侧：装 v1.11.0 验收（扩展重装三步必做）→
导新诊断包发来。**发版提示**：A7 改了收信语义、C5 改了界面，下一轮该出 **v1.11.1**
（只动程序；extension/ 一行没碰，不用 bump EXT_MIN/manifest，用户也不用重装扩展）。
