...
## 状态
**正在出 PLAN_A1C1**（Claude 窗口：A1 工作台聊天持久化 + C1 多会话，一起做）。
步骤：①读 server.py 工作台接口与 DB 结构 → ②读 ui.html 工作台前端 → ③写 PLAN
（DB 新表 + 接口 + 前端会话列表 + e2e 测试方案）→ 存盘推送。
下一件：实现窗口照 PLAN_A1C1.md 施工。
