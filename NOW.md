...
## 状态
**A4 施工完成并已 review**（Claude 窗口一口气做完实现+自查，diff=e7cf451，只动 ui.html 28+/6-，
与 PLAN_A4 逐条一致）。验收全过：演示模式目检（行序/加飞书地址明文🙈/行内换 Telegram 全套重填/
取消不动/Discord 密码框👁/手改名被模板覆盖）；回归 500/500 全绿；content_test 46/16 全绿。
下一件：从 BACKLOG 挑新活（A1-A5 全清了），或按「发版前待办」出一版。
